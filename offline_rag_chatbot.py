...
class HighAccuracyConfig:
    """Yuqori aniqlik uchun optimallashtirilgan konfiguratsiya"""
    CHUNK_SIZE = 250              # Kichikroq chunklar - aniqroq qidiruv
    CHUNK_OVERLAP = 40            # Kam overlap - duplicate kamroq
    MAX_DOCS_PER_QUERY = 12       # Ko'proq context
    MIN_CHUNK_LENGTH = 30         # Minimal chunk uzunligi
    CONFIDENCE_THRESHOLD = 0.3    # Minimal confidence
    MAX_FEATURES = 10000          # TF-IDF features soni
    SVD_COMPONENTS = 400          # Embeddings o'lchami
    DATABASE_PATH = "./high_accuracy_rag.db"
    EMBEDDINGS_PATH = "./high_accuracy_embeddings.pkl"
...
class HybridEmbedder:
    """TF-IDF + SVD + keyword features"""
    
    def __init__(self, max_features: int = 10000, n_components: int = 400):
    ...
    def preprocess_texts(self, texts: List[str]) -> List[str]:
        """Matnlarni preprocessing qilish"""
        processed = []
        for text in texts:
            # Tozalash
...
class AdvancedConfidenceCalculator:
    ...
    def calculate_comprehensive_confidence(self, query: str, results: List[Dict], 
        ...
        if 'combined_score' in results[0]:
            avg_score = np.mean([r.get('combined_score', 0) for r in results])
            confidence_factors.append(min(avg_score, 1.0))
        
        # 2. Top result quality
        top_score = results[0].get('combined_score', 0) if results else 0
        confidence_factors.append(min(top_score, 1.0))
        
        # 3. Query-answer relevance
        query_relevance = self.calculate_query_answer_relevance(query, generated_answer)
        confidence_factors.append(query_relevance)
        
        # 4. Context consistency
        if len(results) > 1:
            consistency = self.calculate_context_consistency(results)
            confidence_factors.append(consistency)
        
        # 5. Answer completeness
        completeness = self.calculate_answer_completeness(query, generated_answer)
        confidence_factors.append(completeness)
        
        # 6. Source diversity
        source_diversity = self.calculate_source_diversity(results)
        confidence_factors.append(source_diversity)
        
        # 7. Keyword coverage
        keyword_coverage = self.calculate_keyword_coverage(query, results)
        ...
        return min(weighted_confidence * 100, 95)
